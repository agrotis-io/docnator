...
test('writed summary', async () => {
  ...
  test('contentList item dont have name', async () => {
    ...
    try {
      await writeSummary(summaryPathTest, [{name: 'carambolas'}, {melancia: 'safadinha'}]);
    } catch(e) {
      errorMessage = e;
    }

    expect(errorMessage).toBe('some Object in content List array dont have a name attribute');
  })
